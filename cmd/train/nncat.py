...
class LitNNCatClassifier(pl.LightningModule):
...
def run_trainer(features=None):
    ...
    root_dir = Path(
        ...
        LitNNCatClassifier.__name__.lower(),
    )
    ...
    for step, (train_indices, test_indices) in enumerate(splits):
        ...
        # Model training
        lit_model = LitNNCatClassifier(x_train.shape[-1], y_train.shape[-1], pos_weight=w[True])
        ...
        if checkpoint_callback.best_model_path != '':
            ...
            lit_model = LitNNCatClassifier.load_from_checkpoint(checkpoint_callback.best_model_path)
